...
- [Recurring findings](recurring-findings.md) — 31 defect classes: self-asserted fields, intra-tenant IDOR, unimplemented MUSTs, rolled-back failure counters, pre-auth KDF in a pooled tx, guards defeated by a stdlib rewrite on the way out, state not bound to the browser, size caps the library ignores.
