...
| B25 | **Release resolution: package version → distro release.** The other half of P3.3's entry condition (ADR-060): map `OpenSSH 4.7p1`/`5.0.51a-3ubuntu5` → `ubuntu804`. Banners carry family, never release, so this is knowledge-pipeline content (P3.1/P3.2 adjacent), not banner extraction. Until it lands, attribution is honestly family-only = unmatched for advisories (ADR-014). **Design measured (session 29–30): [p3.3-release-resolution-design.md](p3.3-release-resolution-design.md)** — session-29 killed the *suffix* bridge; session-30 widened the hardy import to the full set (537 USNs → **190 packages / 589 rows**, from 18/22) and tested the *band*, which was the untested half. **Finding: the upstream band discriminates release** — hardy's `openssh 4.7p1`, `samba 3.0.28a`, `vsftpd 2.0.6`, `mysql 5.0.51a`, `apache2 2.2.8`, `postfix 2.5.1` are each unique in the keyspace, so `4.7p1` ⇒ hardy with no suffix and no rename map. **No new feed needed; the resolver is buildable on the advisory keyspace** (option (a)-via-band: broad per-release import + a light product→package association). Metasploitable is a valid acceptance host — OpenSSH/MySQL/Apache band-match hardy and agree; Samba 3.0.20 and vsftpd 2.3.4 are swapped-in vulnerable builds that abstain (recorded dissent). | `knowledge` / `correlate` | **Unblocked — operator's (a)/(b)/(c) call, data now points at (a).** Prereqs are modest and named: broad per-release USN import (also lifts matching coverage) + a light product→package map. Build is a fresh session. | n/a |
