...
| B39 | **The resolution queue has no operator surface, and ADR-094 now routes every address handover into it.** `asset_resolution_queue` has two Go callers (`Enqueue`, `PendingCount`) and no API route — and `PendingCount` itself has no caller in production code, so the queue is not even visible on Health; nothing marks an item resolved. A queued observation is parked out of the sweep until its item resolves (ADR-094), so a handover's newcomer never gets an asset until someone adjudicates — and no one can. An estate with DHCP churn accumulates parked hosts. | `control/api` / console | A queue screen (item, evidence, candidates, reason) with two verbs — "same host: merge" and "different host: new asset" — that sets `state`/`resolved_at` and lets the parked observation re-sweep; a Health reader for `PendingCount` (Health today shows only the unresolved-observation count, which includes parked rows); and the two ADR-094 trust surfaces that belong on the same screens, because an operator learning the rule by having a job refused is the same class as a silent park: the sighting window (`store.SightingWindow`) as a policy setting tied to the fingerprint cadence rather than a constant, and "sightings needed" on the asset page (which hosts are one sighting short of credentialed work, and when the last one was); and, from ADR-095, an operator verb to CLEAR a pinned exact attribution and re-derive it, because a host that lied within the token grammar is otherwise repaired only by another credentialed read or SQL. | Health — no pending-queue number; Triage — no screen |
