...
| B43 | **MAC and hostname are not in the evidence.** `asset_addresses.mac_address` and `assets.primary_hostname` exist and nothing writes them: the discovery engine emits no MAC (only reachable on the scan point's own segment) and neither engine emits a hostname (reverse DNS, SSH/HTTP banners, mDNS/NetBIOS). They are the natural discriminators between a reimaged host and a different machine on a reused address (B41's classification runs on service continuity and OS agreement alone because these are absent), and MAC is weak by ADR-007 — virtualised, randomised, cloned — so they narrow, never verify. | `engines/discovery` / `correlate` | Discovery emits `mac` when on-segment and `hostname` from reverse DNS; fingerprint emits the banner hostname; correlate records both as weak keys and on the address/asset rows; B41's continuity check then uses them. | Systems — hostname column would finally be real |
