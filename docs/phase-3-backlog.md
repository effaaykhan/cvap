...
| B43 | **MAC and hostname are not in the evidence.** `asset_addresses.mac_address` and `assets.primary_hostname` exist and nothing writes them: the discovery engine emits no MAC (only reachable on the scan point's own segment) and neither engine emits a hostname (reverse DNS, SSH/HTTP banners, mDNS/NetBIOS). They are the natural discriminators between a reimaged host and a different machine on a reused address (B41's classification runs on service continuity and OS agreement alone because these are absent), and MAC is weak by ADR-007 — virtualised, randomised, cloned — so they narrow, never verify. **Schema note (S42, migration 0046):** `asset_resolution_queue_keyed_has_source` requires a service on every non-`ip_window` park and `Enqueue` refuses one without (`ErrKeyWithoutService`, skipped and logged by the sweep). A MAC or a hostname comes from no service, so the day this item emits them the park path refuses unless the CHECK exempts weak key types or they carry the `'unknown'` service the 0046 backfill and the RLS fixture use. Decide that here, before the first weak-key park. | `engines/discovery` / `correlate` | Discovery emits `mac` when on-segment and `hostname` from reverse DNS; fingerprint emits the banner hostname; correlate records both as weak keys and on the address/asset rows; B41's continuity check then uses them. | Systems — hostname column would finally be real |
