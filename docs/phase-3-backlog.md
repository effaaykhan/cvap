...
| B25 | **Release resolution: package version → distro release.** The other half of P3.3's entry condition (ADR-060): map `OpenSSH 4.7p1`/`5.0.51a-3ubuntu5` → `ubuntu804`. Banners carry family, never release, so this is knowledge-pipeline content (P3.1/P3.2 adjacent), not banner extraction. Until it lands, attribution is honestly family-only = unmatched for advisories (ADR-014). **Design pass NOT signed off (session 29): [p3.3-release-resolution-design.md](p3.3-release-resolution-design.md)** — two dev-DB coverage queries refuted the approach: only 1 observed Metasploitable service (MySQL→mysql-dfsg-5.0) intersects the current hardy keyspace, and the revision suffix survives into `services.version` for MySQL only (SSH diverts it to `info`; others carry none). The bare suffix is not release-discriminating anyway — the signal is the package-name era, which needs the product→package map the design tried to avoid. **Re-decision required:** (1) wider hardy import first (a P3.2 data task, also lifts matching coverage); (2) then likely a **release-baseline feed** (the originally-rejected option B) as the only signal that generalizes past the MySQL coincidence. | `knowledge` / `correlate` | **Blocked on the re-decision above** — not the comparators/keyspace (those exist), but the release-signal source. Operator's call: baseline feed vs product→package map vs single-signal-honest. | n/a |
