...
| B44 | **The SSH fingerprint probe verifies no possession.** `internal/engines/fingerprint/ssh.go` reads the host-key blob out of `SSH_MSG_KEX_ECDH_REPLY`, hashes it and abandons the exchange; the signature in the same message is never parsed and no exchange hash is computed, so an SSH host key is an ECHO anyone can present after one `ssh-keyscan`. The TLS probe is not in that state: `tls.Client` with chain verification disabled still verifies the server's signature over the transcript (measured — an echoing server with the victim's certificate and another key is refused), so a certificate fingerprint costs the victim's private key. The ADR-094/096 reviews measured the SSH echo against the merge rule (B40), the corroboration gate and the contested-address park — which is why a contested address parks even the occupant's own agreeing sighting (ADR-096 step 1 §2). | `engines/fingerprint` | Complete the SSH key exchange far enough to verify the host-key signature over the exchange hash and mark the observation `possession_verified`; correlate then grades an unverified SSH key as an echo. Positioned ahead of B40: grading merge evidence by sightings is moot while an SSH sighting is an echo. | Health/Triage — none directly; every identity decision inherits it |
