...
| B39 | **FIRST SLICE LANDED S42 (ADR-097): the confirm verb, the two adjudication verbs, the Identity screen, sightings-needed on the asset page.** ~~The resolution queue has no operator surface, and a rotated host's credentialed scans refuse until an operator can confirm the rotation — a verb that does not exist.~~ **Second slice open:** an `identity.refused` audit event for a refused verb (today a refusal leaves no trace); paging on the queue listing (`limit` is a hardcoded 200, a full page at every cap was measured at 13 MiB, and the resolve path's `pendingAt` read is uncapped inside the write transaction — 936 ms for 3 000 items); the sighting window as a policy setting tied to fingerprint cadence; clearing an operator pin (first a pin WRITER on credential profiles in the API and CLI — `known_hosts` has a reader only); an asset-timeline read of audit events (`ListByResource` has no production caller); clearing a pinned exact attribution (ADR-095). Original entry follows. ADR-096 records a rotated or lapsed SSH key with a provenance the credentialed trust root excludes until an operator pins the key (ADR-091 §4 — `known_hosts` on a credential profile has a reader and no writer in the API or CLI) or confirms the rotation (no verb): every host that reimages or runs `ssh-keygen -A` therefore loses credentialed coverage permanently, announced on Health with nothing behind the chip — and so does every host an attacker answers tcp/22 for across two scans while its own sshd is quiet: the other continuity facts are satisfied by the victim's own banners, the rotation classifies, the trust exclusion holds (measured), and the consequence is not disclosure but permanent loss of credentialed coverage for that host, with no operator remedy. One port for two scans is the cost. The confirm verb is the first deliverable of this item. `asset_resolution_queue` has no API route and no operator verb: nothing an operator can do marks an item resolved (ADR-096 closes an item only when a later scan classifies a rotation; Health now shows the pending count and contested addresses, and `identity.contested`/`identity.rotated` events are keyed to the asset but no route lists an asset's events). A queued observation is parked out of the sweep until its item resolves (ADR-094), so a handover's newcomer never gets an asset until someone adjudicates — and no one can. An estate with DHCP churn accumulates parked hosts. | `control/api` / console | A queue screen (item, evidence, candidates, reason) with two verbs — "same host: merge" and "different host: new asset" — that sets `state`/`resolved_at` and lets the parked observation re-sweep; an asset-timeline read of audit events (`AuditEvents.ListByResource` has no production caller) so the `identity.contested`/`identity.rotated` events ADR-096 keys to the asset are visible where the host is; a **confirm-rotation** verb that re-stamps a `rotation` key's provenance so it can become trust material (until then only an operator pin, ADR-091 §4, lets credentialed work reach a rotated host); and the two ADR-094 trust surfaces that belong on the same screens, because an operator learning the rule by having a job refused is the same class as a silent park: the sighting window (`store.SightingWindow`) as a policy setting tied to the fingerprint cadence rather than a constant, and "sightings needed" on the asset page (which hosts are one sighting short of credentialed work, and when the last one was); and, from ADR-095, an operator verb to CLEAR a pinned exact attribution and re-derive it, because a host that lied within the token grammar is otherwise repaired only by another credentialed read or SQL. | Health — the chip links to the Identity screen; Identity screen and asset-page keys landed (ADR-097) |
| B40 | **Merge evidence is not graded; the trust root is.** ADR-094 keeps a key out of `known_hosts` until two scans have seen it at the address, but `ForAsset`/`LiveByValue` hand a once-seen key to `domain.Resolve` as full ADR-007 merge evidence. An attacker who answers SSH+TLS at a victim's address for one scan can merge their own host onto the victim's asset (measured by the ADR-094 security review); the merge no longer promotes the keys, so the credential does not follow, but the asset holds both hosts. Since ADR-096 the ungraded single-key case has a second face: a host whose one moderate key is live on another asset (its own former asset after a lapse, or a DHCP move) becomes a NEW asset that cannot hold that key (`Record` refuses a second live copy of a value) — a keyless asset that can never re-merge, holding the address, with an empty trust root. Correlate now announces it (`identity.key_held_elsewhere` on the new asset) rather than splitting silently (measured: a returning host became a third, keyless asset); deciding it is this item, and the operator's merge verb (B39) is the interim remedy. | `domain` / `store` | An ADR-007 amendment: whether once-seen keys count as merge evidence at all, or at reduced strength; then `Candidate.Keys` carries the grade and `Resolve` reads it, with a replay test. | Triage — a merged pair reads as one host until then |
| B41 | **RESOLVED S42 (ADR-096).** ~~A key rotation is parked as a handover, and the host disappears.~~ ADR-094 queues a same-service key contradiction at a held address and parks the observation until adjudicated; a reimage or `ssh-keygen -A` produces exactly that shape, so the host's every later sighting is parked, its service row freezes at the pre-rotation version, and no finding is derived — measured by the ADR-094 security re-review (three rotated scans, service still 8.9p1, 3 unresolved, 4 pending). Under the target's control: rotate once, vanish. `PendingCount` had no reader, so it was indistinguishable from a silent drop. **Step 1:** a park is announced — one `identity.contested` audit event on the contested asset's timeline per sweep that parks something new (a repeat sweep of an already-parked group is a no-op and stays silent; a later scan of the host raises again, bounded by the scan cadence), and Health carries the pending item count and the distinct contested addresses. A weak-only sighting at a contested address now waits with the contradiction instead of attaching (the batch-overflow door the security review measured: a host with more observations than one sweep's batch had its overflow attach to the occupant next sweep). The "derive services for a contested observation" half of the original unblocker is DROPPED, not deferred: it is the same door the ADR-094 reviews closed — a newcomer's banner would move the occupant's service rows and findings while its identity is still contested, which is the wrong-merge outcome with a different name. A parked host is visible now; it is still parked. **Step 2:** a contradiction classifies as a rotation and attaches when the newcomer key is seen on two distinct scans, the held key on none since, every previously seen product still answers on its port with the same product, and the OS family agrees; the old key is retired, the new recorded with `rotation` provenance — which the credentialed trust root excludes until an operator pins or confirms it, since a takeover of the SSH port alone is indistinguishable from a rotation on banner data (measured) — the parked items carrying the classified keys close as `rotated` and the parked observations re-enter the sweep. Continuity narrows the window and verifies nothing — every input is public banner data — and the ADR says so. Everything short of all four facts stays queued for B39. | `domain` / `correlate` | — | Health — pending items and contested addresses, linked to the Identity screen (ADR-097) |
| B42 | **"The same service" is the held key's first-seen port, not the port seen now.** `ForAsset` derives a key's source from the copied evidence payload of the observation that first recorded it; the contradiction test in `domain.Resolve` compares on that source, while ADR-094's sightings carry the port seen on each scan. A host stamped 2222 by its first sighting and a newcomer answering on 22 contradict nothing, so the newcomer's key attaches and is recorded; two scans later two distinct host keys qualify at one (address, port) and dispatch now refuses (fail closed). Under ADR-096 the mismatch has two more consequences: the held key's sighting is looked up on its first-seen port, so a host whose sshd moved ports has no sighting on record at the port now and can never classify as a rotation (fails closed, queues for an operator); and `Retire` on that predicate finds nothing, which the rotation path treats as a fault. | `store` / `domain` | Hand `Resolve` one candidate key entry per sighting (address, port) rather than one per key row, so the port under comparison is the one seen now; then the refusal becomes unreachable and can be kept as a guard. | Triage — none; credentialed jobs at such an address are refused until then |
| B43 | **MAC and hostname are not in the evidence.** `asset_addresses.mac_address` and `assets.primary_hostname` exist and nothing writes them: the discovery engine emits no MAC (only reachable on the scan point's own segment) and neither engine emits a hostname (reverse DNS, SSH/HTTP banners, mDNS/NetBIOS). They are the natural discriminators between a reimaged host and a different machine on a reused address (B41's classification runs on service continuity and OS agreement alone because these are absent), and MAC is weak by ADR-007 — virtualised, randomised, cloned — so they narrow, never verify. **BLOCKED until the queue's service constraint widens (S42, migration 0046).** Not a note beside this item — the first line of it fails. `asset_resolution_queue_keyed_has_source` requires a service on every non-`ip_window` park and `Enqueue` refuses one without (`ErrKeyWithoutService`); a MAC and a hostname come from no service, so the first weak key this item emits is parked nowhere: the sweep logs a warning, drops it, and the evidence this item exists to add never reaches the queue an operator reads. The constraint must widen — exempt weak key types, or give them the `'unknown'` service the 0046 backfill and the RLS fixture already use — **before the engine change is attempted**, in its own migration, with `ambiguousServices` (which skips `Strength() < 2`) and the operator verbs re-read against a park that names no port. Doing it the other way round means shipping an engine whose output the store refuses. | `engines/discovery` / `correlate` | Discovery emits `mac` when on-segment and `hostname` from reverse DNS; fingerprint emits the banner hostname; correlate records both as weak keys and on the address/asset rows; B41's continuity check then uses them. | Systems — hostname column would finally be real |
| B44 | **The SSH fingerprint probe verifies no possession — and every trust decision in the system rests on it.** `internal/engines/fingerprint/ssh.go` reads the host-key blob out of `SSH_MSG_KEX_ECDH_REPLY`, hashes it and abandons the exchange; the signature in the same message is never parsed and no exchange hash is computed, so every SSH host key CVAP records is an ECHO — anyone who has seen a host's fingerprint can present it after one `ssh-keyscan`. The TLS probe is not in that state (`tls.Client` verifies the server's signature over the transcript even with chain verification disabled — measured), but **no lab host serves TLS, so today every trust decision rests on echoes.** Two sightings of an echo is two echoes. The two-scan trust root (ADR-094), the rotation classification and the contested-address park (ADR-096), the establishment gate — all of it narrows windows around data that proves nothing about the host holding the private key. The ADRs say so plainly, and the layering stands; but this is the one item that would change the register from "narrows" to "verifies", and it is the same size as B36: complete the key exchange and verify the signature, the way the TLS probe already does. **Positioned as the root**: ahead of B40 (grading merge evidence by sightings is grading echoes), ahead of any further narrowing rule on SSH keys, and named wherever an accuracy or trust claim is made about the observed path. Until it lands, every layer above it is a refinement on unverified input, and the only verification of an SSH host key CVAP has is the operator pin (ADR-091 §4). B39's operator verbs are unaffected: a human decision is not an echo. | `engines/fingerprint` | Complete the SSH key exchange far enough to verify the host-key signature over the exchange hash (the KEX_ECDH_REPLY carries the signature; the exchange hash needs the client's ephemeral key, both KEXINITs and the shared secret — the same work `golang.org/x/crypto/ssh` does before `HostKeyCallback`, which the credentialed engine already runs), and mark the observation `possession_verified`; correlate then grades an unverified SSH key as an echo (a sighting that counts for nothing toward trust or establishment) and the observed trust root is built from verified sightings only. Zero-impact by construction: the exchange completes no authentication (non-negotiable #9). | Every identity and trust surface inherits it; Health should say "observed trust: unverified" until it lands |
| B45 | **The sighting rows carry no protocol.** `asset_identity_key_sightings` is keyed on (key, address, port); `SSHHostKeyFingerprintsAt`, `EstablishedAt`, `LastSeenAt` and `Retire` all read (address, port) with no protocol, while the key's identity unit (its Source, and migration 0045's one-live-key index) is (type, port, protocol). The security review measured a key claimed on `22/udp` contradicting nothing on `22/tcp` — no park, no contest — and then landing in the very sighting row the tcp key earns its trust from, becoming the trust root for the tcp dial after two scans; and `Retire` on port 22 closing a udp sibling as collateral. Closed for now by narrowing the identity path to **tcp only** (`normProtocol`): a non-tcp protocol lifts no identity key. No engine emits anything but tcp. **A latent limitation, not a dormant defect** — the distinction matters because it goes live the moment anything records a non-tcp protocol, and S42 proved that is reachable on UPGRADE, not only by a new probe: a pre-0046 park spelling `22/udp` drove `Retire` (keyed on the port alone) to close, at the same instant, the row the previous loop iteration had just recorded on `22/tcp`, the interval CHECK refused, and `same_host` was impossible at that address for good. `Retire` now carries the protocol; the sightings table still does not. One asymmetry remains and is safe in the closing direction only: `Retire` case-folds the payload's protocol while `asset_identity_keys_one_live_per_port_uidx` does not, so `TCP` and `tcp` are one unit to the retire and two to the index. | `store` / `engines/fingerprint` | Before any UDP or SCTP fingerprint probe lands: add `protocol` to the sightings' primary key and to every predicate that spends a key's trust, written from the key's Source, and widen `normProtocol` in the same change. Positioned with B44: both are the identity unit being narrower than the trust unit. | none until a non-tcp probe exists |
