...
- **OIDC is configurable and not yet implemented.** `tenant_auth_config` holds the issuer, client id, email claim and auto-provisioning choice, and the login path is local-password only. The client is deliberately PUBLIC — authorization code with PKCE, no stored client secret — because a per-tenant secret in a table is the same key-custody problem the encrypted buffer is deferred on. What remains is the discovery, redirect and callback flow. Until it lands, a SaaS tenant has no login path at all, which is the honest state: local auth is on-prem only by three independent conditions.
