...
**Open findings from the discovery session's packet-capture audit, not fixed and not forgotten:**

- **The rate budget counts connect ATTEMPTS; ADR-024's ceiling is in packets.** The kernel sends the initial SYN plus two retransmits inside a 3 s timeout, so a filtered host costs about three packets per charged token — measured at a 2.94x SYN ratio and 200 pps in a 20 ms window against a configured 50. The multiplier is highest against filtered and slow hosts, which is exactly the embedded class the ceiling protects. Either the budget charges for retransmissions, or ADR-024 restates the ceiling as connects/second and records the wire consequence. The second is probably right and it is an ADR amendment, not a code change.

- **A cancelled job records nothing about what it touched.** Coverage lives in `PortsScanned` on the host observation, which is emitted last; measured, nine seconds of scanning and 73 outbound packets produced **zero bytes** of observations on both SIGTERM and SIGKILL. ADR-026 says results are never discarded and a killed job's partial record is what an operator reads to find out what was touched — so this is a real gap, not a nicety. The fix is a periodic partial-coverage row, and it interacts with `KindSent` reclamation, which is also never delivered for a killed job.

- **Safe mode holds every open port's connection for a full second.** `readBanner` waits up to a second for a banner that HTTP never volunteers, with no reduction for `fragile` — against a device with one connection slot that is more damaging than the packet rate. Skipping the unsolicited read on ports known not to volunteer, and capping hold time for fragile assets, is the fix.
